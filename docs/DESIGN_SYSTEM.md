...
| `rounded-lg` | 16 | Dark quiz/result cards — question + scene-context + rating surfaces, the character-select cards, the floating map panel (D-029 Phase B; mockup 14–16) |
...
- The results explore surface — `CareerMap` + `CareerMapField` and the floating `MapContextPanel` (`MapPanelRole` / `MapPanelJob` with `TrajectoryViz`); usage rules in §3.5
- `RoleDetailSheet` — the shared role sheet (RC.org role content + three-axis fit radar), live in `/select`

_(The narrative node map and `FitNote` were deleted at Phase E, D-029; the bubble map, constellation, and `JobOverview` page that followed them went at the career-map merge. The orphaned `SegmentedControl` / `ProgressBar` were deleted in the componentization run.)_
