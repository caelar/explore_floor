...
The realignment that re-centered this spec on the narrative quiz is `docs/knowledge/archive/REALIGNMENT.md`; treat that memo and `DATA_MODEL.md` §17 as the current product, and the exam, conveyor, and robot material below as the parked record. Phase 5 is done: the live model is ARM's three roles.
...
The live experience is a guided "day in the life." After a short researcher-facing landing (§5.1), the user answers a few intro questions (experience, education appetite, salary appetite) and then moves through seven day-in-the-life **scenes** (morning, arriving at school, a class handout, a club lunch, home, homework, a video game). Each scene presents three choices, one tied to each RC.org role, and the user sorts each choice into one of three buckets: **That's me / Kinda me / Not me**. There is no conveyor and no robot build; the sort is a calm, one-card-at-a-time judgement on the shared `BucketSort` mechanic. _(The cut Exam flow reused the same `BucketSort` for its 30 statements.)_ As of step 8 Phase B (D-029), the quiz is re-skinned to the dark system: the intro questions render as a glass question card ("Question N of M", optional prompt, gold answer rows that auto-advance), and each scene runs a **two-beat** — a scene-context card ("Scene N of 7") with a gold **Continue**, then its three choices arrive one at a time ("Choice N of 3") as a rating card sliding in, each rated into the three buckets via uniform gold rating rows. Click-to-rate; the drag path is left dormant. A 2026-06-29 fidelity pass (post-Phase-B) brought three interactions in line with the Claude Design mockup: pressing **Continue** now plays a **slide-up morph** (the scene card compresses and the choices reveal below it, sliding the card up rather than swapping in place); every step **slides horizontally** and the **selected answer/row holds its gold fill through the exit** ("stays lit to show state"); and a persistent **Back** sits centered at the bottom, reversing one step at a time (previous choice → scene intro → previous question, branch-aware) with prior picks shown pre-lit and re-pickable — the research-flagged "no way back" gap. Visual/interaction plan of record: `docs/knowledge/archive/VISUAL_REARCHITECTURE.md`.

The career-map merge (2026-07, Kayla's visuals + career-map branches; ledger `docs/knowledge/CAREER_MAP_REVIEW.md`) added the narrative's **illustrated layer and two screens**: a **character select** (`/character`, between the landing and the quiz — pick who you play as) and a **loading interstitial** (`/loading`, between the quiz and results — a visual motif of the map to come over rotating tabulating copy). The scenes themselves gained illustrated **backgrounds**, the chosen **character**, and per-choice **thought-bubble** illustrations (all mapped in data: `sceneBackgrounds.ts` / `thoughtBubbleIcons.ts`); the quiz surfaces sit on the panel-glass system (`glass-fill-strong` + blur, selected stays solid gold) so they stay legible over the art.

The three roles are ARM's published RC.org career paths: **Technician** (entry), **Specialist** (mid), **Integrator** (planning). Scoring is the pure three-role engine in `lib/categoryScoring.ts`, branch-aware (it only walks the path the user took) and normalized per role against its own max. Results lead with the user's top match and let them compare the others, plus an always-on education/pay fit read. The **Narrative** results are a **three-view system** — the dark **role-cards** screen, **compare**, and the **career map** — led by the cards: a hero (match label, role name, **match %**, ranked **signal bars**), an inline **"why you matched"** breakdown (a plain line on where the % comes from, expandable to what you chose / how school+pay and your scene picks connected / what it means / what you passed on), and two tabs (the role description + an entry-Technician path-up callout + salary/education + duties; competencies + "how to bridge the gap" bridge programs). Prev/next step through the ranked roles. The **career map** (career-map merge, replacing the earlier bubble map + constellation) is one continuous zoomable space: role hubs sized by match % (ties render equal) with their job orbs at overview, a role's cluster at role zoom, one job at job zoom — the latter two carrying a docked glass side rail (the role summary or a compact job summary, whose "Job overview →" opens a restored standalone job-overview page — D-058, reverting the panel-only decision), a phase-local back in the rail header plus an overview-only "Back to your results" exit, and a target-role marker as the outcome-tracking seed. _(The earlier node-map presentation is retired. The cut Exam flow presented a dashboard. Phase 5, D-028, collapsed the study's four categories — Operate/Repair/Program/Plan — into these three roles: Operate + Repair fold into the entry Technician, Program becomes Specialist, Plan becomes Integrator.)_ Full step structure, scoring, and results model are in `DATA_MODEL.md` §17.
...
- The hero carries Kayla's **factory-floor illustration** over the dark canvas as of the career-map merge (Figma 1367:312), with the logo, headline, and CTA centered on it. _(The step-8 Phase A type-led hero, and before it the line-art conveyor sketch with its `DrawSVG` reveal, are the prior states — the dark re-architecture is tracked in `docs/knowledge/archive/VISUAL_REARCHITECTURE.md`.)_
- One primary CTA to start ("Start the story"), a gold pill — an instance of the ecosystem's shared `PillButton` set by the CM-16 ruling (D-055), not a one-off. (`PillButton` is a Figma registry set, `rc-design-system/REGISTRY.md`; in code the landing renders the shared gold-pill recipe inline, same as the results pills.)
...
The most important screen. It must read as a recommendation, not a verdict. _(The live narrative results are the dark **role-cards** screen described in §5.0 and specified in `DATA_MODEL.md` §17 (D-029 Phase C); the cut exam flow presented a dashboard, and the earlier node map is retired as the headline. The robot-on-a-pedestal results below are the classic documented cut. The "recommendation, not a verdict" principle and the specificity payload carry forward verbatim into the live results; only the rendering changed.)_
...
- **The three role accents** are a restrained, kit-only, teal-led set (`categoryAccent.ts`, which now exports `ROLE_ACCENT` — technician→gold, specialist→teal, integrator→orange; finalized at step 8 Phase A, D-029, which retired `arm-blue`). No invented per-screen colors.
- **Motion is modest and state-driven.** Motion (the React library formerly called Framer Motion) carries the UI: screen and flow-step transitions, the bucket-sort drag, the node-map compare swap. GSAP has no live animation as of step 8 Phase A (D-029): the Landing `DrawSVG` reveal, its last flourish, was removed when the Landing went type-led dark; GSAP stays registered as a future seam. Smooth and calm, never frantic. Full rationale in `ARCHITECTURE.md` section 1. _(The conveyor, robotic arm, parts arcing into a robot, and the cinematic build beat are the documented cut, never built.)_
...
- The results screen shows a believable weighted match across the three roles, with the role-stepping compare (prev/next through the ranked roles, signal bars, and the Compare control) working.
...
What remains (the live open questions are tracked in `docs/knowledge/archive/REALIGNMENT.md` §11: define what the match percentage means in one line, frame the entry Technician result as a starting rung rather than a verdict, and settle whether "Kinda me" keeps scoring zero):
