...
- The hero is **type-led on the dark canvas** as of step 8 Phase A (D-029): no scene illustration. The prior placeholder line-art conveyor sketch (and its `DrawSVG` reveal) is parked with the rest of the documented cut; the dark re-architecture is tracked in `docs/knowledge/archive/VISUAL_REARCHITECTURE.md`.
