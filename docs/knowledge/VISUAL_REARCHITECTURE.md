...
### Phase D — Results: compare
- Build `CompareColumns` (two-column hero/why/salary/education/bars + recommendation line), `CompareTargetMenu` (dropdown to switch the compared role among the other two), and the **swap** action. Reuse Phase C components. Do **not** build the removed "compare-pick" chooser (dead code in the mockup).
- *Confirm:* the recommendation-line logic (what makes one role "recommended" over another in a head-to-head).
- *Verify:* E2E opens compare, switches target, swaps.
