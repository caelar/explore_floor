...
- **Pass 3, close:** full gates, `/design-review` rubric pass, D-entries (incl. the CM-12 panel-only decision, the CM-16 hero-button ruling, and the `/character` + `/loading` new-screen flags per the PRD hard rule) + STATUS + this ledger's remaining opens (CM-16, CM-19, team-discussion items), then merge `career-map-merge` to main and hand Kayla the summary. Also reconcile docs that still describe the constellation/job-page architecture (`scene-motion` skill's SVG note, DATA_MODEL/PRD explore-view references) via `/revise-doc`.
