...
- **Next up:** the **componentization run** (`COMPONENTIZATION_RUN.md`, D-041): **Pass 0 is done** — the promotion registry is **ratified** at `rc-design-system/REGISTRY.md` (D-042, 2026-07-03: two button sets, chip fold, nav OverHero kept as a Scroll axis, Icon union sized at 61 names). **Pass 1 is in progress** (2026-07-04): `CtaButton` (`608:7`) + the consolidated `Chip` (`611:12`) are built in the DS library, the CardHead DEF-012 action slot is reconciled, and all 9 extraction sources are staged with bindings intact; the CtaButton radius settled at 8px (Caelan). Remaining: TopNav mega set, PillButton, the form family, the gate, then the two-publish swap/retire sequence and the FIGMA_MAPs — **resume from the [2026-07-04 session note](./sessions/2026-07-04-componentization-pass-1-core-builds.md)**. The **ecosystem run** (`ECOSYSTEM_RUN.md`, D-035) is **done through Pass 7** (Pass 7 closed 2026-07-03, D-040); only its **stretch pass** remains (consumer conversion + atoms tier, only if the last week is calm). Everything feeds the **late-July client handoff** (`HANDOFF_GUIDE.md`); the componentization run is not a handoff gate.
- **Awaiting Caelan:** only the pushes for today's Pass-0 commits (explore_floor docs + rc-design-system `REGISTRY.md`). The nav guidance arrived at Pass 0 and is folded in (the OverHero ruling); the registry is ratified. Prior pushes all landed (explore_floor `48eb014`, robotics_career `ce16a98`, the career_dashboard archive tags).
...
- 2026-07-04 · componentization Pass 1 (in progress): CtaButton + Chip sets built, CardHead slot reconciled, extraction staged and verified → [note](./sessions/2026-07-04-componentization-pass-1-core-builds.md)
