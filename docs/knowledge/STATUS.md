...
- **Next up:** the **ecosystem run** (`ECOSYSTEM_RUN.md`, ratified 2026-07-02, D-035) — executed as single-session passes. Passes 1–3 are done: assets rescued; **`@rc/ui` is live** (`caelar/rc-design-system`, tag `v1`); and **`robotics_career` is stood up** (`caelar/robotics_career` private, main @ `2a89b55`) — Kayla's two branches excavated onto `@rc/ui`, plus the Landing adopted ARM's six-role content (3 robotics + 3 AI, grouped rail). Kayla's Figma file key `k3AjijocJEmzrvlKTd9vJM` is registered **pull-only**; branch closure awaits Caelan's push (archive tags are local in career_dashboard). **Pass 4 is next: the harness port into the UX repo.** Then Passes 5–7 — all feeding the **late-July client handoff** (`HANDOFF_GUIDE.md`). No in-flight build thread in this repo — branch `narrative-v3-realign` is committed and green.
