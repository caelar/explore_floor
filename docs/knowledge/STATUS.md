...
- **Next up:** the **componentization run** (`COMPONENTIZATION_RUN.md`, D-041): **Pass 0 is done** — the promotion registry is **ratified** at `rc-design-system/REGISTRY.md` (D-042, 2026-07-03: two button sets, chip fold, nav OverHero kept as a Scroll axis, Icon union sized at 61 names). **Pass 1's shared tier is built + published (№1)** (2026-07-04, D-043/D-044): all sets exist (CtaButton 8px, Chip, TopNav mega set `636:40`, PillButton `686:2`, form family), raw-hex gate clean, and the **3 dashboard boards are swapped** to the `In+On` two-tier TopNav. The TopNav model was reshaped live (D-044). Remaining for Pass 1: Caelan's **Publish №2** + **old-master retirement** (`262:30`/`184:8`/`183:2`) + **FIGMA_MAPs ×3** — **resume from the [shared-tier session note](./sessions/2026-07-04-pass-1-shared-tier-and-dashboard-swaps.md)**. The **ecosystem run** (`ECOSYSTEM_RUN.md`, D-035) is **done through Pass 7** (Pass 7 closed 2026-07-03, D-040); only its **stretch pass** remains (consumer conversion + atoms tier, only if the last week is calm). Everything feeds the **late-July client handoff** (`HANDOFF_GUIDE.md`); the componentization run is not a handoff gate.
- **Awaiting Caelan:** the DS-library **Publish №2** (pushes the mega set / Chip / form-family updates), a **go-ahead to retire** the 3 old masters (`262:30`/`184:8`/`183:2` — destructive, pairs with Publish №2), and the pushes for the Pass-0/Pass-1 doc commits. Prior pushes all landed (explore_floor `48eb014`, robotics_career `ce16a98`, the career_dashboard archive tags).
