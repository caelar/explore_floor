...
- **Next up:** the **componentization run** (`COMPONENTIZATION_RUN.md`): **Pass 0 + Pass 1 done** (D-042/D-043/D-044) — registry ratified; the shared tier (TopNav mega set, PillButton, CtaButton, Chip, form family) built + published, the 3 dashboard boards swapped to `In+On`, old masters retired, FIGMA_MAPs ×3 written. **Pass 2 (Interest Quiz file) is partial** (D-045, 2026-07-05): a `Components` page + 5 local masters (Chip/StatBox/ChoiceRow/TierBadge/HeroArrow) built and swapped, plus shared TopNav ×9 (guest pill → Guest/G) and CtaButton ×1 — all 9 frames pixel-faithful (`FIGMA_MAP.md` §8). **Next: Pass 2b** — the deferred tail (SignalBars, RoleTabs, cards; the results icon-pills are blocked on a PillButton/CtaButton icon-slot add). The **ecosystem run** (D-035) is done through Pass 7; only its **stretch pass** remains (if the last week is calm). All of it feeds the **late-July client handoff** (`HANDOFF_GUIDE.md`); the componentization run is not a handoff gate.
- **Awaiting Caelan:** the DS-library **final republish** (pushes the Pass-1 adds + the 3 old-master deletions as deprecations) and the pushes for the Pass-1 doc commits (explore_floor `74b06e7`/`7c098e9`, career_dashboard `782a7be`, robotics_career `e1b3b0a`). Prior pushes all landed (explore_floor `48eb014`, robotics_career `ce16a98`, the career_dashboard archive tags).
