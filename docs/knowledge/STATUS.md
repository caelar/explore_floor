...
- **Next up:** the **componentization run** (`COMPONENTIZATION_RUN.md`): **Pass 0 + Pass 1 done** (D-042/D-043/D-044) — registry ratified; the shared tier (TopNav mega set, PillButton, CtaButton, Chip, form family) built + published, the 3 dashboard boards swapped to `In+On`, old masters retired, FIGMA_MAPs ×3 written. **Pass 2 (Interest Quiz file) is partial** (D-045, 2026-07-05): a `Components` page + 5 local masters (Chip/StatBox/ChoiceRow/TierBadge/HeroArrow) built and swapped, plus shared TopNav ×9 (guest pill → Guest/G) and CtaButton ×1 — all 9 frames pixel-faithful (`FIGMA_MAP.md` §8). **Pass 1b done + published** (D-046, 2026-07-05): the shared `PillButton` (`686:2`) + `CtaButton` (`608:7`) sets gained additive leading/trailing icon slots (`Show Icon Left/Right` + `Icon Left/Right`, defaults off, token-bound per variant), verified non-breaking; **republished 2026-07-05** (published sets carry the icon props, importable by key). **Next: Pass 2b** — the deferred local tail (SignalBars, RoleTabs, cards) plus the now-unblocked results icon-pill/CTA swaps + the compare dropdown as a quiz-local master. The **ecosystem run** (D-035) is done through Pass 7; only its **stretch pass** remains (if the last week is calm). All of it feeds the **late-July client handoff** (`HANDOFF_GUIDE.md`); the componentization run is not a handoff gate.
- **Awaiting Caelan:** pushes for the Pass-1/1b doc commits (explore_floor `74b06e7`/`7c098e9`/`713302c`, career_dashboard `782a7be`/`f4a653c`, robotics_career `e1b3b0a`). The DS-library republish is **done (2026-07-05)** — it carried the Pass-1 adds + the 3 old-master deletions (as deprecations) + the Pass-1b PillButton/CtaButton icon slots; the published sets were verified to expose the icon-slot props (importable by key). Prior pushes all landed (explore_floor `48eb014`, robotics_career `ce16a98`, the career_dashboard archive tags).
