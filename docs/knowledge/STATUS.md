...
- **Next up:** **Realignment sweep underway** (`docs/knowledge/REALIGNMENT.md` §9, the plan of record). Branch `narrative-v3-realign`. Done so far: the foundation slice (V3 committed, `/revise-doc`+`doc-steward` ported, tokens kit-aligned, D-024) and **Phase 2, the spec re-centering (D-025)** — every spec doc (`PRD`/`DATA_MODEL`/`DESIGN_SYSTEM`/`ARCHITECTURE`/`ROADMAP`/`CLAUDE.md`/`README`) now leads with the narrative quiz; the conveyor, robot, three-archetype, and Goose-game material is demoted to clearly-marked **"documented cut."** `DATA_MODEL.md` §17 is the primary model (not renumbered). Verified by a 4-skeptic adversarial workflow + `doc-steward`. **Phase 3 done (D-026):** the harness gates now grade the live product — `verifier` + `data-author` headline the §17 flow invariants (classic demoted to a footnote); `goose-game-aesthetic` + `motion-quality` rubrics retired (motion folded into `design-system-compliance`, which is rewritten to the four-category/kit world); a new `results-screen` rubric grades the node-map/dashboard; `scene-motion` rescoped to the live motion. Verified by a 2-skeptic workflow, zero findings. **PAUSED HERE for user go-ahead before Phase 4** (the destructive archive+delete of the dormant classic code: tag `archive/classic-conveyor`, then delete the classic screens/lib/data/tests, drop the three-role hard rule + D-017 carve-out from `CLAUDE.md`, prune the classic types, re-baseline the E2E gate from 7 to ~3. **Load-bearing:** `gsap.ts` is LIVE (keep); **`/src/scene` survives — both placeholders are live-referenced** (`LandingSceneHint` = Landing hero, `RobotPlaceholder` = exam dashboard anchor), so do NOT delete `/src/scene` or remove it from `CLAUDE.md`; rehome the `reduced-motion` E2E onto a live flow. Full corrected plan in `sessions/2026-06-25-realignment-phases-2-3.md`). The old conveyor "feel pass" (`ROADMAP.md` §3) stays **superseded**; the real next product work is the high-fidelity narrative results screen.
