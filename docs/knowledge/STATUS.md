...
- **Next up:** the **componentization run** (`COMPONENTIZATION_RUN.md`, D-041, ratified 2026-07-03): real Figma components + variants across the ecosystem files — nav mega set, button standardization, chip consolidation, form-family promotion, with a bounded code-alignment pass. **Pass 0 (audit + promotion registry) starts in a fresh session**; Caelan brings further nav guidance. The **ecosystem run** (`ECOSYSTEM_RUN.md`, D-035) is **done through Pass 7** (Pass 7 closed 2026-07-03, D-040); only its **stretch pass** remains (consumer conversion + atoms tier, only if the last week is calm). Everything feeds the **late-July client handoff** (`HANDOFF_GUIDE.md`); the componentization run is not a handoff gate.
- **Awaiting Caelan:** only the further nav screenshots/guidance he offered for the componentization run's Pass 0 (one reference is already in: `docs/reference/nav-logged-out-target.png`) — he brings those when Pass 0 starts. Otherwise clear as of 2026-07-03: all pushes landed (explore_floor `48eb014` incl. the D-041 planning commits, robotics_career `ce16a98` with the `.design-sync/` bundle, the career_dashboard archive tags with both old branches deleted and its FIGMA_MAP §4 totals fix), and the Interest Quiz eyeball pass is **done and approved** (its one note became the componentization run, D-041).
