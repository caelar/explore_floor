...
**Resolved 2026-07-15 (D-049) — bound, no republish needed.** The Pass-2 "keep raw" call rested on a **stale premise** ("no library primitive exists" for `#595959`): the DS library already ships `Color/Neutral/Ink 2` = `#595959` (Primitives, key `5e4d25c4…`, id `VariableID:6:9`). So the tidy was a **local rebind in this file**, not a new variable + republish: the SignalBar recessed-track fill (master `127:172`) was bound to the imported `Ink 2`, and the binding propagated to all 8 SignalBar-Off instances (re-scan: **0 raw `#595959` remain**). `--color-white` was **already** bound (aliased to `Color/Neutral/On CTA`; a raw-white scan found 0 unbound whites, 7 bound), so nothing to do there. The compare-Education body fill stays bound to the muted variable. Net: the file carries **no unbound hex** from either token. Because `Ink 2`/`On CTA` were already published, **no DS-library republish is required** (this was the "Figma shows nothing to publish" question — correct, the change is local to the consuming file).
