...
- **Results toolbar pills → PillButton, and icon-CTAs "Continue"/"Role overview" → CtaButton** — **Pass 1b BUILT the icon slots** (2026-07-05, D-046): `PillButton` (§7) + `CtaButton` (career_dashboard §6) now carry `Show Icon Left/Right` + `Icon Left/Right` (defaults off). Remaining gate is **Caelan's republish**, then these swap in Pass 2b. Glyph-per-pill map lives in the Pass 1b spec (`COMPONENTIZATION_RUN.md`).
