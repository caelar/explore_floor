...
/src/data
  ...
  jobs.ts               LIVE (§17, D-029 Phase F) — per-role featured jobs, 4/4/4 (placeholder copy)
  exploreContent.ts     LIVE (§17, Phase G) — the "How you fit" tab narratives (placeholder pending ARM)
  careerMapArt.ts       LIVE (§17, career-map merge) — fixed Figma map geometry (1289:394): per-rank
                        hub/orb/edge art; results fill the rank slots with role name/color/%
  sceneBackgrounds.ts   LIVE (career-map merge) — scene step id → background image path; presentation
                        assets stay out of the flow data so both tune separately
  thoughtBubbleIcons.ts LIVE (career-map merge) — per-scene, per-choice thought-bubble illustrations
                        (fallback to placeholder symbols)
  loadingCopy.ts        LIVE (career-map merge) — the /loading interstitial's rotating status lines
...
/src/lib
  ...
  careerMapLayout.ts    LIVE (§17, career-map merge) — the career-map brain (pure + tested): area-true
                        %-sized hub radii (r = k·√pct, ties equal by construction), edge re-trim onto
                        the live radius, bounds, the pane-aware camera (careerMapPaneRect +
                        careerMapCameraForBoundsInPane), zoom-driven label opacity
  jobTrajectory.ts      LIVE (§17, career-map merge) — resolves a job's prior/current/next ladder
                        rungs (cross-tier aware) for TrajectoryViz
  jobDisplay.ts         LIVE (§17, career-map merge) — salary/education display parsing + fallbacks
  chipFit.ts            LIVE (§17) — one-line chip fitting for the "why you matched" chip row
  nodeLayout.ts         LIVE (§17) — fit-radar geometry (node map deleted Phase E; live in the
                        /select radar)
  index.ts              Barrel export
...
Tests for the live lib functions live in `/src/lib/__tests__/` (102 tests across `categoryScoring`, `screenerFit`, `categoryBreakdown`, `compareRecommendation`, `careerMapLayout`, `jobTrajectory`, `jobOverviewStats`, `chipFit`, `nodeLayout`, and `data-integrity` — the last covers the §17 explore copy block + the per-role job invariants 4/4/4). They run as part of the standard `pnpm test` and are independent of Playwright.
...
The formal-vs-playful A/B was **superseded by the question-structure study** (§17, `DECISIONS.md` D-017) before set B's content was authored. Set B and the `b-` cross-set machinery were removed at that point, leaving `QuestionSetId` as just `'a'`. The QuestionSet shape and set A then survived only because the **classic** flow wrapped set A by reference (§17). Phase 4 deleted the classic flow (D-027), so the question-set apparatus is gone from the live tree. `QUESTION_SET_WORKSHEET.md` was **removed 2026-07-01** — its only revival path (slotting into the classic flow's QuestionSet) died with the classic flow in Phase 4; the A/B-language history lives in `DECISIONS.md` D-016/D-017 and the 2026-06-04 / 06-07 session notes.
...
> **Live headline (D-029 Phase C; compare D; the career map from the career-map merge, 2026-07-18).** The narrative results presentation is the **dark role-cards screen** (`Results/cards/`), with two sibling views: **compare** (Phase D) and the **career map** — Kayla's one-continuous-space zoomable map (overview → role zoom → job zoom), which **replaced the Phase E/F explore surfaces** (the ambient bubble map, the job constellation, and the standalone job-overview page; the first two stay deleted, their content absorbed into the map's docked side rail, and the standalone `JobOverview` page is restored off that rail as of D-058, per the `CAREER_MAP_REVIEW.md` ledger). Per-job content (`src/data/jobs.ts`, 4/4/4 featured jobs — 4 per role) is **placeholder** pending ARM sourcing (the titles are real, from `commonJobTitles`). The old light node map was **deleted in Phase E** (`CategoryResults` / `NodeMap` / `FitNote`); `RoleDetailSheet` + `FitRadar` + `lib/nodeLayout.ts` stay live in the `/select` comparator.

- **Narrative → role cards** (`Results/cards/`): a results experience with an internal view-state (`useResultsNav`: `view` ∈ `cards | compare | map | job-overview`, `roleIndex`, `activeTab`, `expanded`, the compare fields `compareWith` + per-column `compareExpanded`, and the map fields `mapPhase` ∈ `overview | role | job` + `selectedJob` + `targetJobId`, with the verbs `openConstellation` (a role's zoom — the verb name is a Phase-F holdover), `openJob`, `openJobOverview`, `backToConstellation`, `backToJob`, `backToMapOverview`, `setTargetJob`). The results `<main>` is a viewport-height scroll container (sits a constant gap below the nav); `ResultsPanel` splits into a **sticky glass header** (the control bar — Compare / Skip-to-map or Explore — pinned at the sheet top, content scrolls up under it) and a **scrolling body**, with the scroll container's `rounded-t-lg overflow` clipping the corners clean and the body revealing its rounded bottom only at the end (D-029 Phase D polish). The body holds a `RoleHero` (match label + "N of 3"; role name + **match %** both neutral on-dark; ranked **`SignalBars`** from `matchPercentages` with the active role's bar in its accent), an inline **`WhyYouMatched`** breakdown (collapsed line "where your X% comes from" → expanded **01 what you chose / 02 how they connected / 03 what it means / what you passed on**, wired to `categoryBreakdown` + `screenerFit`), and `RoleTabs` (The role: description, an entry-Technician **path-up** callout, salary/education stat cards, `{heading,text}` duties — Skills, path & next steps: ARM **competencies** chips + **bridge-program** rows). Prev/next steps through the ranked roles (resets tab + collapse + scroll-top).
...
- **Narrative → career map** (`Results/cards/CareerMap` + `CareerMapField` + `CareerMapHubLabel` / `CareerMapOrbGlow` / `CareerMapTargetJobOrb`, career-map merge): a truly full-bleed dark canvas reached via the map control ("Skip to your career map" / "Explore your career map"). Fixed Figma map art (`data/careerMapArt.ts`, node 1289:394) draws three rank clusters — hub, four job orbs, dashed branch paths — and the user's results fill the rank slots. **Hubs are area-true sized by match %** (`careerMapHubRadius`: `r = k·√pct`, 40% anchors the Figma hero radius, so ties render equal by construction) and carry distinct glyphs (arm / code / clipboard); edge paths re-trim their hub-side endpoints onto the live radius. **Job dots dim to 0.25 at overview** and brighten on the label zoom ramp or on hover/keyboard-focus with a name reveal. A glass **intro card** ("Your career map", incl. the dots line) dismisses to a persistent **"?" pill** with one settle-based re-expand per map visit (2.5s idle within 1.25× fit; skipped under reduced motion). The decorative `AmbientField` breathes behind everything; all motion reduced-motion-gated.
- **Narrative → map zoom ladder + side rail** (`JobSidePanel` + `JobOverview`, CM-10..12 reverted by D-058): the map's phases are `overview → role → job`, one continuous camera. At role and job zoom a **docked glass rail** (`JobSidePanel`; a bottom sheet on mobile) carries the content as two crossfading bodies: a **role summary** (match %, `SignalBars`, description, compact salary/education chips, a "Jobs in this path" list — click to zoom) at role phase, and a **compact job summary** at job phase, each with a per-phase header back and a teal footer CTA ("Role overview →" at role zoom, "Job overview →" at job zoom). The rail's **"Job overview →"** opens the restored standalone **`JobOverview`** page — a full-bleed `view: 'job-overview'` surface with the full three tabs (overview / skills & competencies incl. bridge programs / "How you fit" with the `TrajectoryViz` ladder via `lib/jobTrajectory`), big salary/education stats, and its own control bar (a Set-target pill + a close-X back to the compact job rail). So exploring a job is two steps: role zoom → rail role summary → a job → rail compact job → "Job overview →" → the large `JobOverview` page. The **pane-aware camera** (`careerMapPaneRect` + `careerMapCameraForBoundsInPane`, CM-11 unchanged) fits the active cluster or orb into the space the rail leaves free, then pan/zoom stay free. Wayfinding: the rail header owns the phase-local back at role/job zoom (a left-aligned, vertically-centered glass pill — "All paths" at role zoom → overview; the role name at job zoom → role zoom), plus an **overview-only "Back to your results"** glass exit pill (top-left, shown only at the map overview phase). The **target-role marker** (`targetJobId`/`setTargetJob` + `CareerMapTargetJobOrb`, unchanged) is the outcome-tracking seed.
- **Narrative → node map** (`Results/category/`) — _deleted in Phase E (D-029)._ Was an Obsidian-style node graph (`CategoryResults` + `NodeMap` + `FitNote`). Its Phase E/F successors (the ambient bubble map, the job constellation + `JobSidePanel`, the standalone `JobOverview` page) were themselves **replaced by the career map and deleted at the career-map merge** (`bubbleLayout` / `constellationLayout` went with them; `JobSidePanel` + `JobOverview` were later restored by D-058, re-hosted on the new map field — the map-rail bullet above). `lib/nodeLayout.ts` geometry stays (used by the `/select` fit radar).
