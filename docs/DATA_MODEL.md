...
/src/lib
  ...
  constellationLayout.ts LIVE (§17, D-029 Phase F) — count-aware polar job-constellation layout
                        (pure + tested): N nodes on an even orbit + center→node edges, fixed
                        CONSTELLATION_VIEW space (reuses polarPoint, not fanPoints)
  nodeLayout.ts         LIVE (§17) — fit-radar geometry (node map deleted Phase E; /select radar +
                        the Phase F constellation reuse polarPoint)
  index.ts              Barrel export
...
Tests for the live lib functions live in `/src/lib/__tests__/` (74 tests across `categoryScoring`, `screenerFit`, `categoryBreakdown`, `compareRecommendation`, `bubbleLayout`, `constellationLayout`, `nodeLayout`, and `data-integrity` — the last now covers the §17 explore copy block + the per-role job invariants 4/4/4). They run as part of the standard `pnpm test` and are independent of Playwright.
...
> **Live headline (D-029 Phase C; compare D; bubble map E; constellation + job overview F).** The narrative results presentation is the **dark role-cards screen** (`Results/cards/`), the first of the mockup's 5-screen system (cards → compare → map → constellation → job overview). **All five screens are now built:** cards (Phase C), compare (Phase D), the ambient bubble map (Phase E), and the **job constellation + job overlay + job-overview page (Phase F)**. Per-job content (`src/data/jobs.ts`, 4/4/4 featured jobs — 4 per role) is **placeholder** pending ARM sourcing (the titles are real, from `commonJobTitles`). The old light node map was **deleted in Phase E** (`CategoryResults` / `NodeMap` / `FitNote`), superseded by the bubble map; `RoleDetailSheet` + `FitRadar` + `lib/nodeLayout.ts` stay live in the `/select` comparator (`polarPoint` is also reused by the Phase F constellation layout).

- **Narrative → role cards** (`Results/cards/`): a results experience with an internal view-state (`useResultsNav`: `view` ∈ `cards | compare | map | selected | job | job-overview`, `roleIndex`, `activeTab`, `expanded`, the compare fields `compareWith` + per-column `compareExpanded`, the map field `fromMap`, and the Phase F explore fields `selectedJob` + the verbs `openConstellation`/`openJob`/`openJobOverview`/`backToConstellation`/`backToJob`/`roleOverview`; Phase C ships `cards`, **D ships `compare`**, **E ships `map`**, **F ships `selected`/`job`/`job-overview`**). The results `<main>` is a viewport-height scroll container (sits a constant gap below the nav); `ResultsPanel` splits into a **sticky glass header** (the control bar — Compare / Skip-to-map or Explore — pinned at the sheet top, content scrolls up under it) and a **scrolling body**, with the scroll container's `rounded-t-lg overflow` clipping the corners clean and the body revealing its rounded bottom only at the end (D-029 Phase D polish). The body holds a `RoleHero` (match label + "N of 3"; role name + **match %** both neutral on-dark; ranked **`SignalBars`** from `matchPercentages` with the active role's bar in its accent), an inline **`WhyYouMatched`** breakdown (collapsed line "where your X% comes from" → expanded **01 what you chose / 02 how they connected / 03 what it means / what you passed on**, wired to `categoryBreakdown` + `screenerFit`), and `RoleTabs` (The role: description, an entry-Technician **path-up** callout, salary/education stat cards, `{heading,text}` duties — Skills, path & next steps: ARM **competencies** chips + **bridge-program** rows). Prev/next steps through the ranked roles (resets tab + collapse + scroll-top).
...
- **Narrative → bubble map** (`Results/cards/ResultsMap` + `BubbleField` + `AmbientField`, D-029 Phase E): a full-bleed dark canvas reached via the Skip-to-map / Explore control. A decorative `AmbientField` (six large blurred, role-tinted orbs that slowly breathe) sits behind a glass "Your results" intro card and the three roles as **bubbles sized by match %** — rank-based (top match largest, high-centre; 2nd/3rd flank below) via the pure `lib/bubbleLayout` (positions per rank, radius scaled by `matchPercentages`). Each bubble fills with its `ROLE_ACCENT.bg`, names the role + match % in `onAccent`, and carries a soft role `glow`; it floats gently (an outer wrapper owns the float so it never fights the inner hover lift). Tapping a bubble **dives** into that role's **constellation** (`openConstellation`, D-029 Phase F; in Phase E it dove to the cards). Full-bleed, so it renders outside the rounded results panel (the `<main>` relaxes for the `map` view). Motion + ambient pulse are reduced-motion-gated.
- **Narrative → job constellation + overlay** (`Results/cards/ResultsConstellation` + `ConstellationField` + `ConstellationNode` + `JobSidePanel`, D-029 Phase F): full-bleed dark canvas reached by diving a map bubble. The role sits at the center (accent-filled, name + match %); its featured jobs (`src/data/jobs.ts`, 4/4/4 — 4 per role) ring it as star nodes on an **even polar layout** (pure `lib/constellationLayout`, reusing `polarPoint`), with dashed center→node edges. A 404px glass `JobSidePanel` on the left shows, on `view: 'selected'`, the role summary (match label, name + %, `SignalBars`, description, salary/education `StatBox`es, a **"Jobs in this path"** list) with a **"Role overview"** footer (→ that role's cards, marks `fromMap`) and an **"All paths"** back (→ map). Tapping a node (or a list row) opens **`view: 'job'`**: the clicked node fills + glows (others dim) and the panel body **crossfades** (its own `AnimatePresence`) to the job summary (eyebrow "Job in {role}", title, summary, salary/education, "What you'll do"), with a gold **"Job overview"** footer (→ the overview page) and a role-name back (→ constellation). `selected`+`job` are **one keyed `motion.div`** in `ResultsExperience` so the constellation never remounts between them. Node float + entrance + hover are reduced-motion-gated.
- **Narrative → job overview** (`Results/cards/JobOverview` + `TrajectoryViz`, D-029 Phase F): the standalone job page, in the rounded scroll panel (reuses `ResultsPanel`). Sticky control bar with a **Back** (→ job overlay) and an **inert "Set as target role"** pill (chrome only, no real wiring per scope). Eyebrow + title, then three tabs (**local** `useState`, not `nav.activeTab`): **Job overview** (description, salary/education `StatBox`es, "What you'll do" responsibilities), **Skills & competencies** (ARM `competencies` 2-col grid + per-job skill `Chip`s + the **"How to close the gap"** bridge-program rows), **How you fit** ("You as a {noun}" + role description + a static **`TrajectoryViz`** Technician→Specialist→Integrator ladder, current role lit). Per-job salary/education fall back to the role-level `roleDetails` when a `Job` has no override.
