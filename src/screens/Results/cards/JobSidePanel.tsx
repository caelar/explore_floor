...
// The glass left rail on the career map (restored, D-029 Phase F). One shell, two bodies: on
// `selected` it shows the role summary (match label, name + %, signal bars, description,
// salary/education, and the "jobs in this path" list); on `job` it swaps to the open job's
// summary. The body crossfades via its own AnimatePresence so the map behind it never remounts.
// Header back + footer CTA differ per view — "Job overview →" opens the large standalone
// JobOverview page. Tokens only.
...
// The header back control is a glass platter (same pill recipe as the map's overview exit and the
// other results controls) so it reads as chrome, not body text. Left-aligned, vertically centered.
const backRow =
  'inline-flex h-control-lg items-center gap-space-1 rounded-full border border-glass-border bg-glass-fill-strong px-space-3 font-body text-small font-medium text-text-on-dark-muted backdrop-blur-panel transition-colors hover:text-text-on-dark';
...
export function JobSidePanel(props: JobSidePanelProps) {
  ...
  return (
    ...
    >
      {/* header back row */}
      <div className="flex shrink-0 items-center border-b border-glass-border px-space-4 py-space-3">
        {view === 'selected' ? (
