---
name: scene-motion
description: Use when authoring or editing animation — the flow-step transitions, the click-to-rate bucket-sort micro-interactions, the dark results screens' idle/ambient loops + bubble/constellation/compare layout, or screen transitions. Encodes the hard Motion-vs-GSAP ownership boundary, the useGSAP discipline, and the shared motion tokens so the two engines never fight. Trigger on tasks touching GSAP, Motion (motion/react), AnimatePresence, layout animations, variants, timelines, or DrawSVG.
---
...
> **Scope note (realignment + step 8).** The conveyor scene, robotic arm, live-building robot, and cinematic build beat this skill was originally written for are the **documented cut** (never built). All live motion is **Motion (`motion/react`)**; **GSAP has zero live uses today** (it registers at app start as a dormant seam — see below). The live motion, all of it modest: the Landing type reveal, the flow step-to-step transitions (direction-aware), the click-to-rate card/row micro-interactions, and the dark results screens' idle "breathe" loops + bubble/constellation/compare layout. The ownership rule and `useGSAP` discipline below still govern if scene motion ever returns.
...
## Ownership map
- **Motion (`motion/react`, formerly framer-motion)** owns everything live: screen + flow-step transitions (`AnimatePresence`, **direction-aware** via a `custom` prop so Back slides opposite to Forward — see `FlowRunner`), card enter/exit, the intro-choice stagger (`MCQuestion`), the **click-to-rate** buckets (`BucketSort` — tap/Enter, **not** drag), hover/tap micro-interactions, the results **bubble / constellation / compare** entrance + `layout` + idle-float loops, and the global `prefers-reduced-motion` gate. Pattern: declarative `motion.div` / `motion.svg` with variants.
...
Per-screen (live): **Landing** — Motion type reveal + CTA. **Flow** — Motion owns the direction-aware step transitions, the click-rating card/row micro-interactions, and the intro-choice stagger. **Results** — Motion owns the role-cards reflow, the bubble map + job constellation (entrance, `layout`, idle float), and the compare swap.
...
- `breathe` (seconds): the multi-second idle-loop bases for the results screens — `orb` 7 · `bubble` 5 · `node` 4 · `sparkle` 2.4 (each consumer adds its own per-index/per-rank increment + stagger).
...
## SVG rendering
Live SVG geometry: the `/select` comparator's three-axis (triangle) **fit radar** (`lib/nodeLayout.ts`), the results **ambient bubble map** (`BubbleField`), and the **job constellation** (`ConstellationField`, `lib/constellationLayout.ts`). Named motion variants live alongside the component that uses them. _(Documented cut: the old narrative node map was deleted at step-8 Phase E — `nodeLayout.ts` now feeds only the `/select` radar; the `/src/scene/` assembly-line hierarchy — `Factory`, `ConveyorBelt`, `RoboticArm`, `Bin`, `Robot` — was never built.)_
